...
//nobita code
import  static x10.visit.X10PrettyPrinterVisitor.classDetails;
import  static x10.visit.X10PrettyPrinterVisitor.graphInfo;
import  static x10.visit.X10PrettyPrinterVisitor.graphInfoInitial;
import  static x10.visit.X10PrettyPrinterVisitor.setInfo;
import  static x10.visit.X10PrettyPrinterVisitor.objInfo;
import  static x10.visit.X10PrettyPrinterVisitor.lastGraphInfo;
import  static x10.visit.X10PrettyPrinterVisitor.currClass;
import  static x10.visit.X10PrettyPrinterVisitor.currMethod;
...
import  static x10.visit.X10PrettyPrinterVisitor.objParaGraphInfo;
...
//      Object[] components = new Object[1 + typeArgs.size() * 3 + args.size() + classTypeArgs.size() * 3];
    ...
    public void generateMethodDecl(X10MethodDecl_c n, boolean boxPrimitives) {
        ...
        /* Nobita code - for identifying the object para of a method*/
        	int parNo = lineNo;
        	...
        	if (iterationCount == 0) {
	        	...
	        	while (it.hasNext()) {
	        		...
	        		if (ci.name.equals(n.name().toString())  && ci.uniqueId == null) {
	        			if (ci.methodPara == null) {
	        				ci.methodPara = new LinkedList<ClassInfo>();
	        				path = true;
	        				ci1 = ci;
	                        ci.methodNoIn = currMethod.peek().lineNo;
	        			}
	        			methodPara = ci.methodPara;
	        			break;
	        		}
	        	}
        	}
        ...
        /* Nobita code - String var to store the unique pattern */
        String uniqValue = "";	
        if (path) {
        ...
        for (int i = 0; i < n.formals().size(); i++) {
            ...
            if (path) {
	            ...
	            if(!((varType.equalsIgnoreCase("Long")) || (varType.equalsIgnoreCase("Float")) || (varType.equalsIgnoreCase("String")) || (varType.equalsIgnoreCase("FileReader")) || (varType.equalsIgnoreCase("Printer")) || (varType.equalsIgnoreCase("Random")) || (varType.equalsIgnoreCase("FileWriter")) || 
	            		...
	            		(varType.equalsIgnoreCase("Array")) || (varType.equalsIgnoreCase("DistArray")) || (varType.equalsIgnoreCase("Region")))) {
	            	if (methodPara != null) {
	            		//classifier => ObjPara90; name=> name
	            		parNo++;
	            		String parIndex = ("Obj-Par"+parNo);
	            		ClassInfo ci = new ClassInfo(parIndex, varType, f.name().toString());
	            		methodPara.add(ci);
	            	}
	            }
            }
        ...
        if (hasNativeAnnotation) {
            ...
            if (n.body() != null) {
                ...
                /* Nobita code - inter-procedural analysis */
                // to every method initialize the structure of the formal parameters
                if (currMethod.size() > 0) {
	                ...
	                if (setMethodInfo != null) {
	                	...
	                	if (placeInfo != null) {
	                		setDetails = placeInfo.get(temp2.lineNo);
	                	}
	                }
	                ...
	                if (methodInfo2 != null) {
	                	...
	                	if (placeInfo != null) {
	                		objDetail = placeInfo.get(temp2.lineNo);
	                	}
	                }
	                ...
	                if (iterationCount > 0) {
	                	...
	                	while (it.hasNext()) {
	                		...
	                		if (ci.classifier.equalsIgnoreCase("method") && ci.name.equals(n.name().toString()) && ci.uniqueId != null && ci.uniqueId.equalsIgnoreCase(uniqValue)) {
	                			methodPara = ci.methodPara;
	                			break;
	                		}
	                	}
	                ...
	                HashMap<String, String> shaObjDetails = new HashMap<String, String>();
	                // call the function here
	                if (iterationCount > 0 && methodPara != null) {
	                	createObjSetForPara(varInfo, setDetails, objDetail, methodPara, shaObjDetails, 0);
	                }
	                
	                if (!graphInfoInitial.containsKey(currClass.peek().lineNo)) {
	                	graphInfoInitial.put(currClass.peek().lineNo, new HashMap<Integer, HashMap<String, LinkedList<EdgeRep>>>());
	                }
	                graphInfoInitial.get(currClass.peek().lineNo).put(currMethod.peek().lineNo, new HashMap<String, LinkedList<EdgeRep>>());
	                graphInfoInitial.get(currClass.peek().lineNo).put(currMethod.peek().lineNo, varInfo);
	                if (iterationCount > 0 && ci != null) {
	                	ci.methodNoIn = currMethod.peek().lineNo;
	                } 
                }
                /* Nobita code */
...
//    		if (!mi.returnType().isVoid()) {
    ...
    /* Nobita code - inter-prodecural method */
    public void createObjSetForPara(HashMap<String, LinkedList<EdgeRep>> varInfo, HashMap<String, HashMap<String, HashSet<String>>> setDetails,
    		HashMap<String, ObjNode> objDetail, LinkedList<ClassInfo> methodPara, HashMap<String, String> shaObjDetails, int lineNo) {
    	Iterator it = methodPara.iterator();
    	while (it.hasNext()) {
    		ClassInfo ci = (ClassInfo)it.next();
    		
    		String paraName = ci.classifier;
    		LinkedList<EdgeRep> ller = objParaGraphInfo.get(paraName);
    		if (ller != null) {
    			
    			varInfo.put(ci.name, new LinkedList<EdgeRep>());
    			LinkedList<EdgeRep> llerN = varInfo.get(ci.name);
    			
    			Iterator it1 = ller.iterator();
    			while (it1.hasNext()) {
    				EdgeRep er = (EdgeRep)it1.next();
    				if (er.desName.equalsIgnoreCase("Obj-null")) {
    					EdgeRep er1 = new EdgeRep("P", "Obj-null", true);
    					llerN.addLast(er1);
    				}
    				else {
    					if(er.desName.charAt(0) == 'S') {
    						if (shaObjDetails.containsKey(er.desName)) {
    							EdgeRep er1 = new EdgeRep("P", shaObjDetails.get(er.desName), true);
        						llerN.addLast(er1);
    						}
    						else {
    							lineNo++;
    							String objName = er.desName+"-P"+lineNo;
        						EdgeRep er1 = new EdgeRep("P", objName, true);
        						llerN.add(er1);
        						shaObjDetails.put(er.desName, objName);
        						
        						//object creation
        						ObjNode on = new ObjNode(objName,ci.type);
        						objDetail.put(objName, on);
        						...
        						//call the fn here
                        		createObjSetForPara_Field(varInfo, setDetails, objDetail, shaObjDetails, lineNo, er.desName, objName);
    						}
    	...
    	/*
    	if (varInfo != null) {
			System.out.println("Inside the emitter");
    ...
    /* Nobita code - inter-procedural analysis */
    void createObjSetForPara_Field(HashMap<String, LinkedList<EdgeRep>> varInfo, HashMap<String, HashMap<String, HashSet<String>>> setDetails,
    		HashMap<String, ObjNode> objDetail, HashMap<String, String> shaObjDetails, int lineNo, String srcObj, String desObj) {
    	
    	LinkedList<EdgeRep> llerSrc = objParaGraphInfo.get(srcObj);
    	String srcType = objDetail.get(desObj).objType;
    	
    	if (llerSrc != null) {
    		
        	if (!varInfo.containsKey(desObj)) {
        		varInfo.put(desObj, new LinkedList<EdgeRep>());
        	}
        	LinkedList<EdgeRep> llerDest = varInfo.get(desObj);
    		
    		Iterator it = llerSrc.iterator();
    		while (it.hasNext()) {
    			EdgeRep er = (EdgeRep)it.next();
    			
    			if (er.desName.equalsIgnoreCase("Obj-null")) {
    				EdgeRep er1 = new EdgeRep("F", "Obj-null", er.fieldName, true);
    				er1.edgeName = er.edgeName;
    				llerDest.addLast(er1);
    			}
    			else {
    				if(er.desName.charAt(0) == 'S') {
    					if (shaObjDetails.containsKey(er.desName)) {
    						EdgeRep er1 = new EdgeRep("F", shaObjDetails.get(er.desName), er.fieldName, true);
    						er1.edgeName = er.edgeName;
        					llerDest.addLast(er1);
    					}
    					else {
    						lineNo++;
    						String objName = er.desName+"-P"+lineNo;
    						EdgeRep er1 = new EdgeRep("F", objName, er.fieldName, true);
    						er1.edgeName = er.edgeName;
        					llerDest.addLast(er1);
        					shaObjDetails.put(er.desName, objName);
        					
        					LinkedList<ClassInfo> llci = classDetails.get(srcType);
        					String desType = "";
        					...
        					if (llci != null) {
        						...
        						//the object
        						ObjNode on = new ObjNode(objName,desType);
        						objDetail.put(objName, on);
