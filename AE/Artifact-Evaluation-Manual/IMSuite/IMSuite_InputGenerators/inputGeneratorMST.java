...
/** 
 * inputGeneratorMST implements an input generator for generation
 ...
 */
public class inputGeneratorMST 
{
	/** 
		...
		if(choice == 0 || choice == 1) {
			...
			for(i=0; i<size; i++)		
				for(j=i+1; j<size; j++) {
					...
					if(randm < 0)
						randm = -randm;
					weightMat[i][j] = weightMat[j][i] = randm;
				}
			
			for(i=0; i<size; i++)							// Printing weight matrix
				for(j=0; j<size; j++)
					output.write("\n" + weightMat[i][j]);	
			output.close();
		}	
...
class SparseGraph 
{
	static int GRSIZE = 16, count = 0, total = 0, array[], parentArray[], maxChild;
	...
	static void print(String fileName, Random r)	throws Exception {
		...
		int randm;	boolean tflag;
		for(i=0; i<GRSIZE; i++)		
			for(int j=i+1; j<GRSIZE; j++) {
				do {
					tflag = false;
					randm = r.nextInt();
					for(int k=0; k<GRSIZE; k++) {	
						for(int l=0; l<GRSIZE; l++)
							if(weightMat[k][l] == randm) {
								tflag = true;
								break;
							}
	...
	SparseGraph(int type, int parent, Random r) {
		...
		if(type == 0) {
			...
			for(int i = 0; i < GRSIZE; i++)	
				array[i] = i;
			inputGeneratorMST imt = new inputGeneratorMST();
			for(int i = 0; i < 2 * GRSIZE; i++) {
				int x = imt.PseudoRandomGen(GRSIZE, r);
				int y = imt.PseudoRandomGen(GRSIZE, r);
				int temp = array[x];
				array[x] = array[y];
				array[y] = temp;
			}
		...
		if(childNum != 0) {
			...
			for (int i = 0; i < childNum; i++)
				children[i] = new SparseGraph(1, ID, r);
		}	
	}
}
