...
/** 
 * inputGeneratorLeaderElectDP implements an input generator for generation
 ...
 */
public class inputGeneratorLeaderElectDP 
{
	/** 
	...
	public static void main(String []args) throws Exception {
		...
		boolean flag=false;
		String fileName = "inputleader_elect_dp.txt";
...
class SparseGraph 
{
	...
	static void print(String fileName, Random r)	throws Exception {
		...
		inputGeneratorLeaderElectDP ikc = new inputGeneratorLeaderElectDP();                
               	for(i=0; i<GRSIZE*4; i++) {	
			x = ikc.randomGen(GRSIZE, r);
               	        y = ikc.randomGen(GRSIZE, r);
               	        temp = a[x];
               	        a[x] = a[y];
               	        a[y] = temp;
               	}
	...
	SparseGraph(int type, int parent, Random r) {
		...
		if(type == 0) {
			...
			for(int i = 0; i < 2 * GRSIZE; i++) {
				int x = ikc.randomGen(GRSIZE, r);
				int y = ikc.randomGen(GRSIZE, r);
				int temp = array[x];
				array[x] = array[y];
				array[y] = temp;
			}
