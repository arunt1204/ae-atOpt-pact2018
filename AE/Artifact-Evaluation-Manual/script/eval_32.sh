...
if [ "$4" = "timestamp" ]; then
	...
	if [ "$BENCHMARK" = "all" ]; then
		...
		echo "Executing for bfsDijkstra:"
		$BASHPATH speedup_32.sh "bfsDijkstra" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for byzantine:"
		$BASHPATH speedup_32.sh "byzantine" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for dijkstraRouting:"
		$BASHPATH speedup_32.sh "dijkstraRouting" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for dominatingSet:"
		$BASHPATH speedup_32.sh "dominatingSet" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for kcommitte:"
		$BASHPATH speedup_32.sh "kcommitte" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for leader_elect_dp:"
		$BASHPATH speedup_32.sh "leader_elect_dp" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for leader_elect_hs:"	
		$BASHPATH speedup_32.sh "leader_elect_hs" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for leader_elect_lcr:"
		$BASHPATH speedup_32.sh "leader_elect_lcr" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for mis:"
		$BASHPATH speedup_32.sh "mis" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for mst:"
		$BASHPATH speedup_32.sh "mst" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
		echo "Executing for vertexColoring:"
		$BASHPATH speedup_32.sh "vertexColoring" $HOST1 $HOST2 $HOME_PATH | tee -a ./Results/32_cores/all_kernels_speedup.txt
	elif [ "$BENCHMARK" = "bfsBellmanFord" ] || [ "$BENCHMARK" = "bfsDijkstra" ] || [ "$BENCHMARK" = "dominatingSet" ] || [ "$BENCHMARK" = "kcommitte" ] || [ "$BENCHMARK" = "leader_elect_dp" ] || 	[ "$BENCHMARK" = "mis" ] || [ "$BENCHMARK" = "mst" ] || [ "$BENCHMARK" = "dijkstraRouting" ] || [ "$BENCHMARK" = "vertexColoring" ] || [ "$BENCHMARK" = "byzantine" ] || [ "$BENCHMARK" = 	"leader_elect_hs" ] || [ "$BENCHMARK" = "leader_elect_lcr" ]; then
