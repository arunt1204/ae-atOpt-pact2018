				...
				nodeSet(aNode).pair.u = index;          nodeSet(aNode).pair.v = index;
					...
					finish {
						for(c in 0..(nodeSet(aNode).children.size()-1)) {
                                			async {
                                				var child:Int = nodeSet(aNode).children.get(c);
                                				var cpt: Point = Point.make(child);
                                        			setSignal(cpt, START);
								...
								atomic
								{
                               	...
                               	for(var i:Int=0; i<nodes; i++)              // Checking for other neighbours of the non-leaf node,
                               	{
        	        		...
        	        		for(i in 0..(nodeSet(aNode).children.size()-1)) {
        						...
        						atomic { 
        							...
        							casync(aNode) = casync(aNode) + 1;
        	               				...
        	               				var cpt:Point = Point.make(nodeSet(aNode).children.get(i));
        ...
        def sendJoinSignal(val sender:Int, val reciever:Point) {
        	...
        	at(D(reciever)){
	        	...
	        	atomic {
        			...
        			catom(reciever) = catom(reciever) + 1;
