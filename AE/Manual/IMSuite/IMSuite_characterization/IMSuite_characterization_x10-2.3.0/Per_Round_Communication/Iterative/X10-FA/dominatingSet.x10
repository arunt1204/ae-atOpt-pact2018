...
public class dominatingSet 
{
	...
	def initialize() {
		...
		finish {
			for(i in D) {
				async at(D(i)) {
					var j:Int; var length:Int; var count:Int = vertexSet(i).neighbours.size; var retArray:Array[Int];
					var flag:Boolean;
					var index:Int = i.operator()(0);
					...
					for(j=0; j<vertexSet(i).neighbours.size; j++)
						if(vertexSet(i).neighbours(j) != index) {
							...
							for(var k:int=0; k<retArray.size; k++) {
								flag = false;
	...
	def createDominatingSet() {
		...
		do {
			...
			/** Each iteration marks a round. */
			messCount();
			cmess = DistArray.make[Int](D);
		}while(check);
	}
