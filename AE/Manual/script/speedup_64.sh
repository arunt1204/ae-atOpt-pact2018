...
if [ $# -eq 3 ]; then
	echo "Speedup of IMSuite Benchmark kernel $KERNEL.x10 on one node system"
	host1=$2
	PATH_FOLDER=$4
	echo "$host1" > hostnames.txt
fi
...
./x10c++ -x10rt sockets $PATH_FOLDER/IMSuite/IMSuite_timing_speedup/IMSuite_timing_x10-2.5.0/IMSuite_Iterative/X10-FA-2.5.0/$KERNEL_FILE
...
do
	...
	BASE_TIME=`timeout 1800 sudo ./a.out -in $PATH_FOLDER/IMSuite/IMSuite_Input/$INPUT_FILE`
	echo "Execution time: $BASE_TIME"
	echo " "
	...
	AT_OPT_TIME=`timeout 1800 sudo ./a.out -in $PATH_FOLDER/IMSuite/IMSuite_Input/$INPUT_FILE`
	echo "Execution time: $AT_OPT_TIME"
	echo " "
